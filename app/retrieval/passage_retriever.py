...
class PassageRetriever:
    ...
    def _passes_filters(self, entry: _IndexedPassage, query: Query) -> bool:
        f = query.filters
        if f.pov is not None and entry.pov != f.pov:
            return False
        if f.is_quest is not None and entry.is_quest != f.is_quest:
            return False
        if entry.work_id in f.exclude_works:
            return False
        for dim, (lo, hi) in f.score_ranges.items():
    ...
    def _score(self, entry: _IndexedPassage, query: Query) -> float:
        ...
        Hard filters have already been applied in :meth:`_passes_filters`;
        this score is a soft tie-breaker. For each requested ``score_range``
        dim, compute how close the entry's value is to the range midpoint
        (normalized by the half-width). Values within half the half-width
        of midpoint count as "met"; farther ones shed 0.2 each.
        """
        score = 1.0
        f = query.filters
        for dim, (lo, hi) in f.score_ranges.items():
            value = entry.score_for(dim)
            if value is None:  # should have been filtered out earlier
                score -= 0.2
                continue
            mid = (lo + hi) / 2.0
            half = max((hi - lo) / 2.0, 1e-6)
            # Distance to midpoint as a fraction of the half-width.
