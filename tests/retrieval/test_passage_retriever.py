...
async def test_filter_pov_returns_only_matching(retriever: PassageRetriever):
    ...
    assert {r.metadata["work_id"] for r in results} == {"quest_c"}
...
async def test_score_range_narrows_results(retriever: PassageRetriever):
    ...
    keys = {(r.metadata["work_id"], r.metadata["passage_id"]) for r in results}
    assert keys == {
        ("novel_a", "p01"),
        ("novel_a", "p02"),
        ("quest_c", "p01"),
...
async def test_exclude_works_drops_them(retriever: PassageRetriever):
    q = Query(filters=QueryFilters(exclude_works={"novel_a", "novel_b"}).to_dict())
    results = await retriever.retrieve(q, k=10)
    assert {r.metadata["work_id"] for r in results} == {"quest_c"}
...
async def test_k_limits_result_count(retriever: PassageRetriever):
    ...
    assert (results[0].metadata["work_id"], results[0].metadata["passage_id"]) == ("novel_a", "p01")
    assert (results[1].metadata["work_id"], results[1].metadata["passage_id"]) == ("novel_a", "p02")
...
async def test_score_is_tiebreaker_by_midpoint(retriever: PassageRetriever):
    ...
    results = await retriever.retrieve(q, k=10)
    assert [(r.metadata["work_id"], r.metadata["passage_id"]) for r in results] == [
        ("novel_a", "p02"),
        ("novel_a", "p01"),
    ]
...
async def test_fallback_to_combined_labels(tmp_path: Path):
    ...
    assert (results[0].metadata["work_id"], results[0].metadata["passage_id"]) == ("novel_b", "p01")
