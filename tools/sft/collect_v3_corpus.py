"""Phase-3 kickoff: collect the LoRA v3 SFT corpus across four diverse seeds.

Inherits the v2 structure (flush-after-every-update, try/except per update,
SFT collection on, N candidates per scene) and adds a fourth seed —
``folk_horror`` — featuring a cartographer named Idris who arrives to find
the village not on his own maps. The narrator tilts acoustic + interoceptive
and voice samples deliberately span the 1-4-word register up through
long cascading observational sentences.

Output layout::

    data/sft/v3/<quest_id>/<quest_id>/u<u>_s<s>_<trace>.json
    data/stress_v3/<quest_id>_run_log.jsonl

Usage::

    uv run python tools/sft/collect_v3_corpus.py \\
        --seed noir|intrigue|heist|folk_horror --updates 10 --n 4

Or run all four back-to-back::

    uv run python tools/sft/collect_v3_corpus.py --seed all --updates 10 --n 4
"""
...
FOLK_HORROR_SEED: dict = {
    "entities": [
        {"id": "village", "entity_type": "location", "name": "Tre-Halwyn"},
        {"id": "player", "entity_type": "character", "name": "Idris",
         "data": {"voice": {
             "vocabulary_level": "plain-precise",
             "sentence_length_bias": "varied",
             "directness": 0.6,
             "emotional_expression": "quiet-unsettled",
         }}},
        {"id": "warden", "entity_type": "character", "name": "Nesta",
         "data": {"voice": {"vocabulary_level": "village-vernacular",
                            "sentence_length_bias": "clipped",
                            "directness": 0.4,
                            "emotional_expression": "guarded"}}},
        {"id": "boy", "entity_type": "character", "name": "Corran",
         "data": {"voice": {"vocabulary_level": "plain",
                            "sentence_length_bias": "short",
                            "directness": 0.2,
                            "emotional_expression": "watchful"}}},
    ],
    "plot_threads": [
        {"id": "pt:main", "name": "The Village Not On My Maps",
         "description": "Idris the cartographer has reached Tre-Halwyn with five commissioned surveys that all disagree — and a village that does not appear on any of them. Someone in the village knows why, and someone has decided he will not ask.",
         "arc_position": "rising", "priority": 8},
        {"id": "pt:b", "name": "The Road Behind",
         "description": "The road Idris walked in on does not match the road he remembers walking, and he is not sure when it changed.",
         "arc_position": "rising", "priority": 5},
    ],
    "themes": [
        {"id": "t:unplace", "proposition": "a place that refuses to be mapped is a place that has already decided about you",
         "stance": "exploring"},
        {"id": "t:hospitality", "proposition": "the welcome that never asks your business is the one that already knows it",
         "stance": "exploring"},
    ],
    "narrator": {
        "pov_character_id": "player",
        "pov_type": "third_limited",
        "worldview": "an unsettled observer — a cartographer who arrives to find the village not on his own maps",
        "editorial_stance": "precise, professionally composed, privately unmoored",
        "sensory_bias": {"auditory": 0.35, "interoceptive": 0.3,
                         "visual": 0.15, "tactile": 0.1,
                         "kinesthetic": 0.05, "olfactory": 0.05},
        "attention_bias": [
            "the road behind",
            "doors with iron",
            "what the villagers don't explain",
        ],
        "voice_samples": [
            # 1-4 words — pure interoceptive register
            "Wrong. All of it.",
            # short village-vernacular dialogue, clipped
            "\"We don't say that name after dark,\" Nesta told him. \"And you won't either, if you're staying.\"",
            # medium — auditory detail with a small unease
            "The bell in the tower rang four times when the light was still full, and no one in the square looked up.",
            # long cascading observational — the cartographer's eye + the interoceptive bleed
            "The road behind the village did not match the road he remembered walking in on — the stones on the left had the wear of a westward trade route he had surveyed eight years ago in a different valley entirely, the stones on the right had no wear at all, as though the road had been laid the week before he arrived, and somewhere between the last waymark and the first iron-bound door he had stopped noticing which of his feet was doing the counting.",
        ],
    },
}
...
FOLK_HORROR_ACTIONS = (
    # observation
    "I stand in the square and try to place the village by the shape of its rooftops.",
    "I unroll the westernmost of my five surveys on the low wall and compare the ridgeline to the horizon.",
    "I walk the perimeter once, slow, counting doors and noting which have iron on them.",
    # asking a villager too directly
    "I find the warden Nesta at the well and ask her, plain as I can, why the village is on none of my maps.",
    "\"What happened to the cartographer who came before me?\" I ask Nesta, not looking away.",
    "I ask the boy Corran where the road behind the village goes, and I want a real answer.",
    # waiting at an uneasy table
    "I take the offered seat at the inn table and eat what's put in front of me without asking what it is.",
    "I wait while the room empties around me and I am the last of the supper.",
    "I let the silence sit after Nesta says \"You'll sleep in the loft tonight,\" and I don't fill it.",
    # refusing to be told where not to go
    "Nesta tells me not to walk past the old chapel. I thank her and note the direction.",
    "\"I'll go where my work takes me,\" I say, calmly, and roll my surveys back into their tube.",
    "When Corran is sent to fetch me back from the lane, I pretend not to have seen him.",
    # entering the marked place despite the warning
    "I step through the iron-bound door of the old chapel at the hour Nesta said I should not.",
    "I cross the chalk-line on the chapel's threshold with my lamp lit and my notebook open.",
    "I set my lamp on the altar and begin surveying the chapel's interior aloud, the way I was taught.",
)
...
SEEDS: dict[str, dict[str, Any]] = {
    "noir": {"seed": NOIR_SEED, "actions": NOIR_ACTIONS, "quest_id": "noir_v3",
             "genre": "low-fantasy noir"},
    "intrigue": {"seed": INTRIGUE_SEED, "actions": INTRIGUE_ACTIONS,
                 "quest_id": "intrigue_v3", "genre": "political intrigue"},
    "heist": {"seed": HEIST_SEED, "actions": HEIST_ACTIONS,
              "quest_id": "heist_v3", "genre": "science-fantasy heist"},
    "folk_horror": {"seed": FOLK_HORROR_SEED, "actions": FOLK_HORROR_ACTIONS,
                    "quest_id": "folk_horror_v3",
                    "genre": "folk horror / quiet dread"},
}
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(prog="collect_v3_corpus")
    ...
    ap.add_argument("--sft-root", type=Path,
                    default=ROOT / "data" / "sft" / "v3",
                    help="Root of SFT collection output (v3 subtree).")
    ap.add_argument("--workdir", type=Path,
                    default=Path("/tmp/collect_v3_corpus"))
    ap.add_argument("--log-root", type=Path,
                    default=ROOT / "data" / "stress_v3")
...
async def main_async() -> None:
    ...
    out = Path(args.log_root) / "collect_v3_summary.json"
